...
export default async function DashboardPage() {
  ...
  return <>
    ...
    {viewer.stats.total === 0 ? <div className="setup-panel"><h2>{t(locale, "Connect your first device")}</h2><p>{t(locale, "The account is ready. Run this command where your Codex or Claude Code logs live.")}</p><CommandCopy /></div> : <>
      <div className="profile-stats dashboard-stat-grid"><div><small>{t(locale, "ALL TIME")}</small><strong>{formatTokenCount(viewer.stats.total)}</strong></div><div><small>{t(locale, "THIS MONTH")}</small><strong>{formatTokenCount(viewer.stats.month)}</strong></div><div><small>{t(locale, "TODAY")}</small><strong>{formatTokenCount(viewer.stats.today)}</strong></div><div><small>{t(locale, "MONTHLY RANK")}</small><strong>{rank ? `#${rank.rank}` : "—"}</strong>{rankChange !== 0 && <span className="rank-change" data-up={rankChange > 0 || undefined}>{rankChange > 0 ? `↑ ${rankChange}` : `↓ ${Math.abs(rankChange)}`} {t(locale, "since previous snapshot")}</span>}</div></div>
