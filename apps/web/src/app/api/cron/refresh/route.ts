...
export async function POST(request: Request) {
  ...
  for (const [period, start] of Object.entries(periods)) for (const source of ["all", "codex", "claude-code"]) {
    const filter = source === "all" ? "" : `AND ud.source='${source}'`;
    await db.batch([db.prepare("DELETE FROM leaderboard_snapshots WHERE period=?1 AND source=?2").bind(period, source), db.prepare(`INSERT INTO leaderboard_snapshots (id,period,source,user_id,rank,processed_tokens,codex_tokens,claude_tokens,active_days,percentile,generated_at)
      SELECT lower(hex(randomblob(16))),?1,?2,user_id,ROW_NUMBER() OVER (ORDER BY total DESC,active_days DESC,first_reached ASC),total,codex,claude,active_days,100.0*ROW_NUMBER() OVER (ORDER BY total DESC,active_days DESC,first_reached ASC)/COUNT(*) OVER (),?3 FROM (
        SELECT ud.user_id,SUM(input_tokens_total+output_tokens_total) total,SUM(CASE WHEN ud.source='codex' THEN input_tokens_total+output_tokens_total ELSE 0 END) codex,SUM(CASE WHEN ud.source='claude-code' THEN input_tokens_total+output_tokens_total ELSE 0 END) claude,COUNT(DISTINCT utc_date) active_days,MIN(last_event_at) first_reached FROM usage_daily ud JOIN profiles p ON p.user_id=ud.user_id WHERE ud.utc_date>=${start} ${filter} AND ud.quarantined=0 AND p.is_public=1 AND p.show_rank=1 AND ud.trust_level!='imported' GROUP BY ud.user_id
