...
export async function generateMetadata({ params }: { params: Promise<{ handle: string }> }): Promise<Metadata> {
  ...
  return { title: locale === "zh" ? `${p.displayName} 的 AI Token 档案` : `${p.displayName}'s AI Token Portfolio`, description: locale === "zh" ? `${p.displayName} 在 Codex、Claude Code 与 WorkBuddy 中已处理 ${publicTotal}。` : `${p.displayName} has processed ${publicTotal} across Codex, Claude Code, and WorkBuddy.`, alternates: languageAlternates(`/u/${p.handle}`, locale), openGraph: { images: [`/share/${p.handle}/profile.svg?theme=obsidian`] } };
}
...
export default async function ProfilePage({ params }: { params: Promise<{ handle: string }> }) {
  ...
  return <>
    ...
    <section className="shell profile-content">
      ...
      {isOwner ? <article className="panel profile-share"><div className="panel-head"><h2>{locale === "zh" ? "分享此档案" : "Share this portfolio"}</h2><span>{achievementFor(p.processedTokens, p.activeDays, p.codexTokens, p.claudeTokens, locale)}</span></div><ShareThemeGallery activeDays={p.activeDays} claudeTokens={p.claudeTokens} codexTokens={p.codexTokens} workbuddyTokens={p.workbuddyTokens} displayName={p.displayName} handle={p.handle} history={p.history} locale={locale} models={p.models} percentile={p.percentile} rank={p.rank} showExactTokens={p.showExactTokens} showRank={p.showRank} sources={p.sources} tokens={p.processedTokens} /><div className="share-gallery-foot"><LocaleLink href={localePath("/methodology", locale)} locale={locale}>{locale === "zh" ? "了解统计方式" : "How this is measured"}</LocaleLink></div></article> : <LocaleLink className="profile-rank-cta" href={localePath("/", locale)} locale={locale}><span className="profile-rank-cta-icon"><Trophy size={34} /></span><span className="profile-rank-cta-copy"><small className="eyebrow">{locale === "zh" ? "你的使用量，也能被看见" : "MAKE YOUR USAGE VISIBLE"}</small><strong>{locale === "zh" ? "你也来加入排名。" : "Join the leaderboard."}</strong><span>{locale === "zh" ? "连接 Codex、Claude Code 或 WorkBuddy，只同步每日汇总用量，生成你的个人趋势与可选公开排名。" : "Connect Codex, Claude Code, or WorkBuddy, sync daily aggregates only, and build your own trends and optional public rank."}</span></span><span className="profile-rank-cta-action">{locale === "zh" ? "前往首页开始统计" : "Start from the homepage"}<ArrowRight size={18} /></span></LocaleLink>}
    </section>
