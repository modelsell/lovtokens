...
export async function generateMetadata({ params }: { params: Promise<{ handle: string }> }): Promise<Metadata> {
  ...
  const title = locale === "zh" ? `${p.displayName} 的 AI Token 档案` : `${p.displayName}'s AI Token Portfolio`;
  const description = locale === "zh" ? `${p.displayName} 在 Codex、Claude Code 与 WorkBuddy 中已处理 ${publicTotal}。` : `${p.displayName} has processed ${publicTotal} across Codex, Claude Code, and WorkBuddy.`;
  const image = `/share/${p.handle}/social.png?theme=obsidian`;
  return { title, description, alternates: languageAlternates(`/u/${p.handle}`, locale), openGraph: { title, description, url: `/u/${p.handle}`, type: "website", images: [{ url: image, width: 1200, height: 630, type: "image/png", alt: title }] }, twitter: { card: "summary_large_image", title, description, images: [image] } };
}

export default async function ProfilePage({ params, searchParams }: { params: Promise<{ handle: string }>; searchParams: Promise<{ share?: string }> }) {
  ...
  return <>
    ...
    <ShareLandingTracker contentId={p.handle} contentKind="profile" conversionSelector=".profile-rank-cta" />
  </>;
}
