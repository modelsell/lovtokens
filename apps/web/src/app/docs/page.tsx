...
export default async function DocsPage() {
  ...
  return <>
    <PageHero eyebrow={t(locale, "Collector docs · Node 20+")} title={t(locale, "One command, visible boundaries.")} description={t(locale, "Connect Codex and Claude Code usage from macOS, Linux, or Windows without uploading the conversations behind the count.")} />
    <article className="shell prose-shell">
      ...
      <table><tbody>
        ...
        <tr><td><code>lovtokens auto-sync install</code></td><td>{t(locale, "Opt in to a 30-minute user-level task.")}</td></tr>
      ...
      <p>{locale === "zh" ? <>Codex：<code>$CODEX_HOME/sessions</code> 和 <code>archived_sessions</code>。Claude Code：已知的 <code>~/.claude/projects</code> 兼容目录。指向这些根目录之外的符号链接会被跳过。</> : <>Codex: <code>$CODEX_HOME/sessions</code> and <code>archived_sessions</code>. Claude Code: known <code>~/.claude/projects</code> compatible directories. Symlinks escaping these roots are skipped.</>}</p>
