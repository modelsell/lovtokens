...
export default async function HomePage() {
  ...
  return <>
    ...
    <section className="hero agent-hero"><div className="shell hero-grid"><div className="hero-copy"><span className="eyebrow">{locale === "zh" ? "CODEX + CLAUDE CODE 用量看板" : "CODEX + CLAUDE CODE USAGE"}</span><h1>{locale === "zh" ? <>看清你的<em>AI 编程用量。</em></> : <>See your<em>AI coding usage.</em></>}</h1><p className="hero-lead">{locale === "zh" ? "LovTokens 自动汇总 Codex 与 Claude Code 的 Token 使用量，生成个人趋势、模型分布和可选排行榜。" : "LovTokens turns Codex and Claude Code token usage into personal trends, model breakdowns, and an optional leaderboard."}</p><div className="hero-assurance"><span><ShieldCheck size={13} /> {locale === "zh" ? "不上传对话和代码" : "No conversations or code"}</span><span><EyeOff size={13} /> {locale === "zh" ? "默认私密" : "Private by default"}</span><span><Clock3 size={13} /> {locale === "zh" ? "可选每 30 分钟更新" : "Optional 30-minute updates"}</span></div></div><div className="hero-visual" id="agent-register"><AgentRegistrationGuide content={handoff} documentUrl={registrationDocumentUrl} locale={locale} serverUrl={root} /></div></div></section>
