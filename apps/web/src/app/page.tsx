...
import { ArrowRight, BarChart3, Bot, Clock3, Code2, Copy, EyeOff, FileJson2, ShieldCheck } from "lucide-react";
...
export default async function HomePage() {
  ...
  return <>
    ...
    <section className="hero agent-hero"><div className="shell hero-grid"><div className="hero-copy"><span className="eyebrow">CODEX + CLAUDE CODE + WORKBUDDY</span><h1>{t(locale, "See your")}<em>{t(locale, "AI coding usage.")}</em></h1><p className="hero-lead">{t(locale, "LovTokens turns Codex, Claude Code, and WorkBuddy token usage into personal trends, model breakdowns, and an optional leaderboard.")}</p><div className="hero-assurance"><span><ShieldCheck size={13} /> {t(locale, "No conversations or code")}</span><span><EyeOff size={13} /> {t(locale, "Private by default")}</span><span><Clock3 size={13} /> {t(locale, "Optional hourly updates")}</span></div></div><div className="hero-visual" id="agent-register"><AgentRegistrationGuide content={handoff} documentUrl={registrationDocumentUrl} locale={locale} serverUrl={root} /></div></div></section>
    ...
    <section className="section shell"><div className="section-head"><div><span className="eyebrow">{t(locale, "Three steps")}</span><h2>{t(locale, "Registration becomes a conversation.")}</h2></div><p>{t(locale, "Users no longer need to understand the CLI, device codes, or settings pages first. The agent executes; the user still controls every identity and visibility choice.")}</p></div><div className="how-grid"><article className="how-card"><span>01 / COPY</span><Copy /><h3>{t(locale, "Copy the handoff")}</h3><p>{t(locale, "Copy only the document URL and one execution sentence instead of the full guide.")}</p></article><article className="how-card"><span>02 / READ</span><Bot /><h3>{t(locale, "The agent reads the MD")}</h3><p>{t(locale, "The production document contains identity checks, privacy boundaries, execution steps, and completion rules.")}</p></article><article className="how-card"><span>03 / COMPLETE</span><Clock3 /><h3>{t(locale, "Register and sync")}</h3><p>{t(locale, "Create the account and device, run the first sync, and optionally install an hourly task with daily update checks.")}</p></article></div></section>
    <section className="privacy-band"><div className="shell privacy-grid"><div><span className="eyebrow">{t(locale, "A deliberate boundary")}</span><h2>{t(locale, "Your content stays yours.")}</h2><p>{t(locale, "LovTokens counts usage without building a second copy of your work. It never uploads prompts, replies, code, file paths, repository names, or API keys.")}</p><LocaleLink className="section-link" href={link("/privacy")} locale={locale}>{t(locale, "Read the privacy contract")} <ArrowRight size={15} /></LocaleLink></div><div className="privacy-proof"><div className="privacy-list"><span><ShieldCheck size={17} /> {t(locale, "Aggregates only")}</span><span><EyeOff size={17} /> {t(locale, "Private by default")}</span><span><BarChart3 size={17} /> {t(locale, "Absolute-value upserts")}</span><span><FileJson2 size={17} /> {t(locale, "Open schema")}</span></div><div className="open-source-proof"><div className="open-source-proof-head"><Code2 size={21} /><div><strong>{t(locale, "The collector is open source.")}</strong><span>AGPL-3.0</span></div></div><p>{t(locale, "Want to inspect the upload first? Run this command before connecting or syncing:")}</p><code>npx lovtokens@latest show-data</code><div className="open-source-links"><LocaleLink href={link("/privacy#inspect-upload")} locale={locale}>{t(locale, "See upload code and privacy details")} <ArrowRight size={14} /></LocaleLink><a href="https://github.com/modelsell/lovtokens" rel="noreferrer" target="_blank">{t(locale, "View the open-source project")} <ArrowRight size={14} /></a></div></div></div></div></section>
  </>;
}
