...
export function ShareCardPreview({
  ...
  return (
    <article className={`share-card share-${theme}`}>
      ...
      <div className="share-card-distributions"><MiniDistribution empty={locale === "zh" ? "智能体私密" : "Agents private"} max={maxSource} rows={sources.slice(0, 2).map((row) => ({ label: row.source === "claude-code" ? "Claude" : "Codex", tokens: row.tokens }))} title={locale === "zh" ? `智能体分布 · Codex ${codexPercent}%` : `AGENT MIX · CODEX ${codexPercent}%`} /><MiniDistribution empty={locale === "zh" ? "模型私密" : "Models private"} max={maxModel} rows={models.slice(0, 3).map((row) => ({ label: row.model, tokens: row.tokens }))} title={locale === "zh" ? "模型分布" : "MODEL MIX"} /></div>
