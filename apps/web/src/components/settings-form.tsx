...
export function SettingsForm({ initial, fields, locale = "en" }: { initial: Values; fields: Array<{ key: string; label: string; help: string; type: "text" | "toggle" }>; locale?: Locale }) {
  ...
  return <div className="settings-grid">{fields.map((field) => <label className="setting-row" key={field.key}><span><strong>{field.label}</strong><small>{field.help}</small></span>{field.type === "text" ? <input className="text-input" value={String(values[field.key] || "")} onChange={(event) => setValues({ ...values, [field.key]: event.target.value })} /> : <input checked={Boolean(values[field.key])} onChange={(event) => setValues({ ...values, [field.key]: event.target.checked })} type="checkbox" />}</label>)}<button className="primary-button" onClick={save} type="button">{t(locale, "Save changes")}</button>{message && <p className="form-message">{message}</p>}</div>;
}
