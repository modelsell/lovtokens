...
export function PublicAchievementShelf({ achievements, locale }: { achievements: PublicAchievement[]; locale: Locale }) {
  ...
  const cards = achievements.flatMap((item) => {
    if (item.kind === "behavior") {
      ...
      return badge ? [{ item, title: badge.title, subtitle: badge.targetLabel || badge.mark, mark: badge.mark, image: badge.image, club: false }] : [];
    }
    ...
    const detail = milestoneClubForTarget(Number(item.period));
    return [{
      item,
      title: milestoneClubText(detail, locale).title,
  ...
  return <section className="profile-achievements" id="achievements">
    ...
    <div className="profile-achievement-grid">
      {cards.map(({ item, title, subtitle, mark, image, club }) => {
        const content = <>
        ...
        const className = `profile-achievement-item${club ? " profile-achievement-club" : ""}`;
